...
// Example to use dynamic grammar
...
int main(int argc, char* argv[]) {
  if(argc != 4 && argc != 6){
    std::cerr << "Usage: " << argv[0] << " <ws_url> <grammar_file> <wav_path> [ <user> <password> ]" << std::endl;
    std::cerr << "   eg: " << argv[0] << " ws://127.0.0.1:8025/asr-server/asr gram/phone.gram audio/phone-1937050211-8k.wav" << std::endl;
    std::cerr << "  eg2: " << argv[0] << " wss://contact/cpqd/and/request/a/key/ /path/to/gram /path/to/audio.wav myusername mypassword" << std::endl;
    return -1;
  }
  ...
  try {
    // pass grammar inline
    lm = LanguageModelList::Builder().addInlineGrammar(str_gram).build();
